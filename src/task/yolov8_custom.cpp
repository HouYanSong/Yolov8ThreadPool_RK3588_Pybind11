...
nn_error_e Yolov8Custom::LoadModel(const char *model_path, const int input_w, const int input_h, const int class_num, const std::vector<std::vector<int>>& mapSizeVec)
{
    ...
    for (int i = 0; i < mapSizeVec.size(); i++) {
        for (int j = 0; j < mapSizeVec[i].size(); j++) {
            mapSize[i][j] = mapSizeVec[i][j];
        }
    }
...
nn_error_e Yolov8Custom::Postprocess(const cv::Mat &img, std::vector<Detection> &objects, float objectThreshold, float nmsThreshold)
{
    void *output_data[6];
    for (int i = 0; i < 6; i++)
    {
    ...
    if (want_float_)
    {
        ...
        yolo::GetConvDetectionResult((float **)output_data, DetectiontRects, input_w, input_h, objectThreshold, nmsThreshold, class_num, mapSize);
        // NN_LOG_INFO("use float version postprocess");
    }
    else
    {
        // 使用量化版本的后处理，只能处理量化的模型
        yolo::GetConvDetectionResultInt8((int8_t **)output_data, out_zps_, out_scales_, DetectiontRects, input_w, input_h, objectThreshold, nmsThreshold, class_num, mapSize);
...
nn_error_e Yolov8Custom::Run(const cv::Mat &img, std::vector<Detection> &objects, float objectThreshold, float nmsThreshold, std::string preprocess_mode)
{

    // letterbox后的图像
    cv::Mat image_letterbox;
    // 预处理，支持opencv或rga
    Preprocess(img, preprocess_mode, image_letterbox);
    // 推理
    Inference();
    // 后处理
    Postprocess(image_letterbox, objects, objectThreshold, nmsThreshold);
