...
namespace yolo
{
    ...
    // static int class_num = 10; // 类别数
    static int strides[3] = {8, 16, 32};
    // static int mapSize[3][2] = {{100, 100}, {50, 50}, {25, 25}};
...
#define ZQ_MIN(a, b) ((a) < (b) ? (a) : (b))
    ...
    std::vector<float> GenerateMeshgrid(int mapSize[3][2])
    {
    ...
    int GetConvDetectionResultInt8(int8_t **pBlob, std::vector<int> &qnt_zp, std::vector<float> &qnt_scale,
                                   std::vector<float> &DetectiontRects, int input_w, int input_h, float objectThreshold, float nmsThreshold, int class_num, int mapSize[3][2])
    {
        static auto meshgrid = GenerateMeshgrid(mapSize);
        int ret = 0;
    ...
    // 浮点数版本
    int GetConvDetectionResult(float **pBlob, std::vector<float> &DetectiontRects, int input_w, int input_h, float objectThreshold, float nmsThreshold, int class_num, int mapSize[3][2])
    {
        static auto meshgrid = GenerateMeshgrid(mapSize);
        int ret = 0;
